...
import (
	...
	"github.com/mobile-directing-system/mds-server/services/go/permission-svc/endpoints"
	"github.com/mobile-directing-system/mds-server/services/go/permission-svc/eventport"
...
const kafkaGroupID = "mds-permission-svc"
...
func Run(ctx context.Context) error {
	...
	logger, err := logging.NewLogger("permission-svc", c.LogLevel)
	if err != nil {
	...
	readyCheck := func(ctx context.Context) error {
		...
		// Check Kafka topics.
		eg.Go(func() error {
			err := kafkautil.AwaitTopics(egCtx, c.KafkaAddr, event.PermissionsTopic, event.UsersTopic)
